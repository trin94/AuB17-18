...
\begin{document}
    ...
    \begin{equation*}
        \begin{multlined}
            ...
            + (1 - p(n))^2 \cdot p(n) \cdot (f(n) + g(n))
            + ...
        \end{multlined}
    \end{equation*}
    \begin{equation*}
        \begin{multlined}
            = \left( f(n) + g(n)) \cdot p(n) \cdot \sum^{\infty}_{i=1}(1-p(n))^i < \frac{f(n) + g(n)}{p(n)} \right)
        \end{multlined}
    \end{equation*}
    ...
    \begin{figure}[htb]
        \centering
        \includepic{lec_03_b}{0.5}
        \caption{Einfügen eines Elements "`an der richtigen Stelle"'}
    \end{figure}
